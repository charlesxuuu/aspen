...
/* Modified by Silvery Fu; print out format (all in the same line): */
/* Socket (Package) Cores (PowerPlane0) Graphic (PowerPlane1) */
/* LLC, Memory Controller (Package - PowerPlane0 - PowerPlane1) */
/* DRAM */ 
...
/* MSR code                    */
/*******************************/
static int rapl_msr(int core, int cpu_model, char *iperf_cmd) {

	pid_t iperf_pid;
	int fd, status, end_id;
	...
	if((iperf_pid = fork()) == -1 )
	{
		...
		// printf("%d\n", (int)iperf_pid);
		time(&start);
		end_id = waitpid(iperf_pid, &status, 0);
		time(&end);
		...
		for(j=0;j<total_packages;j++) {
			...
			printf("%.6f",
				(package_after[j]-package_before[j])/duration);

			result=read_msr(fd,MSR_PP0_ENERGY_STATUS);
			pp0_after[j]=(double)result*cpu_energy_units[j];

			// Report the core portion
			printf(" %.6f",
				(pp0_after[j]-pp0_before[j])/duration);
			...
			// Report he graphics
			printf(" %.6f",
				(pp1_after[j]-pp1_before[j])/duration);
		}

		// Report the LLC memory controller
		printf(" %.6f",
			(package_after[j] - package_before[j] - pp0_after[j] + pp0_before[j] - pp1_after[j] + pp1_before[j])/duration);
		...
		// Report the DRAM
		printf("DRAM: %.6f W ",
			(dram_after[j]-dram_before[j])/duration);
	}
	printf("\n");	
	close(fd);
	}
...
int main(int argc, char **argv) {
	...
	while ((c = getopt (argc, argv, "c:i:hmps")) != -1) {
		switch (c) {
		case 'c':
			core = atoi(optarg);
			break;
		case 'i':
			...
			break;
		case 'h':
			printf("Usage: %s [-c core] [-i iperf_cmd] [-h] [-m]\n\n",argv[0]);
			printf("\t-c core : specifies which core to measure\n");
			printf("\t-i iperf_cmd : specifies which iperf cmd to run\n");
			printf("\t-h      : displays this help\n");
			printf("\t-m      : forces use of MSR mode (brief)\n");
	...
	result=rapl_msr(core,cpu_model,iperf_cmd);
